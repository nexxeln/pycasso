...
def generate_rectangle_art(path: str):
    ...
    for i, point in enumerate(points):
        ...
        if i == len(points) - 1:
            ...
            point_2 = points[i + 1]
        rectangle_xy = (point_1, point_2)
        colour_factor = i / (len(points) - 1)
        line_colour = fade_colour(start_colour, end_colour, colour_factor)
        thickness += scale_factor
        overlay_draw.rectangle(rectangle_xy, outline=line_colour, width=thickness)
...
if __name__ == "__main__":
    for i in range(10):
        generate_rectangle_art(f"rectangle_art/rectangle_examples/image_{i}.png")
